...
- **macOS Memory Overhead (Colima):** Because macOS cannot run Linux containers natively, Colima must spin up a background Linux VM. While the actual containers (`warp`, `tailscale`, `routing-fix`) only consume roughly `~75MB` of RAM combined, the Colima VM requires a base memory allocation just to boot the Linux kernel and Docker daemon. We have successfully tested the VM running stably at `0.5 GB` (512MB) of RAM (`colima start --memory 0.5`), making the entire stack incredibly lightweight even with the macOS hypervisor tax.
